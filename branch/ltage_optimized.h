...
#define BANK_TABLE_SIZE 2048
#define GLOBAL_HISTORY_LENGTH 640
#define NO_OF_BANKS 12
#define NO_OF_BITS_BIMODAL_COUNTER 2
#define NO_OF_BITS_BANK_COUNTER 3 
#define BANK_INDEX_SIZE 11
...
const int tag_sizes[] = {7,7,8,8,9,10,11,12,12,13,14,15} ;
//const int tag_sizes[] = {9,10,12,13,14,15,15,15,14,12,10,9};
...
class Tables { 
    public : 
     ...
     void add_new_entry_miss( int bank_chosen  , int taken , uint64_t ip ) { 
          ...
          if ( count ) { 
               ...
               for ( int i = bank_chosen + 1 ; i <=NO_OF_BANKS  ; i++ ) {  
                if ( !banks[i-1]->getindex(ind[i])->u ) {
                      if ( rand_bank == k ) {    
                          banks[i-1]->update( ind[i] , banks[i-1]->get_tag(ip) );
                          break ;
                      }
                      k += 1 ; 
                  }
     ...
     void findPred(uint64_t ip) {
          bank_pred = bimodal_table->getindex(ind[0])->msb ;  
          bank_chosen = 0 ; 

          for ( int i = NO_OF_BANKS-1  ; i >= 0 ; i-- ) {  //bank_chosen  = i + 1 
              if ( banks[i]->getindex(ind[i+1])->tag == banks[i]->get_tag(ip) ){ //banks[i]->get_tag(ip) ) {
                   bank_chosen = i+1 ; 
...
public:
    ...
    uint8_t predict(uint64_t ip) {
        ...
        //tag find start 
        temp = GHR;
        int tag = (ip & ~(~0 << 8)) ^ (temp &= CSR1_mask).to_ulong();
        temp = GHR;
        temp >>= 8;
        tag = tag ^ ((temp &= CSR2_mask).to_ulong() << 1);
        tables.findPred(ip);
        int z = tables.banks[0]->get_tag(ip); 
        //cout<<"pred : "<<tag<<" "<<( z )<<endl ; 
        ////cout<<"Start"<<endl ; 
        ////cout<<tag<<endl ; 
    ...
    void last_branch_result(uint64_t ip, uint8_t taken) {
        ...
        // X <= 3
        if ( bank_pred != taken) {
            tables.add_new_entry_miss( bank_chosen , taken , ip );
        }
